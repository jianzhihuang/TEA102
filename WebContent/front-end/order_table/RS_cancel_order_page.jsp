...
<body>
	...
	<div class="btn_1">
		<a
			href="<%=request.getContextPath()%>/front-end/order_table/RS_all_order_page.jsp">
			<input type="button" value="全部訂單" name="按鈕名稱">
		</a>
	</div>

	<div class="btn_2">
		<a
			href="<%=request.getContextPath()%>/front-end/order_table/RS_complete_order_page.jsp">
			<input type="button" value="已成立訂單" name="按鈕名稱">
		</a>
	</div>

	<div class="btn_3">
		<a
			href="<%=request.getContextPath()%>/front-end/order_table/RS_cancel_order_page.jsp">
			<input type="button" value="已取消訂單" name="按鈕名稱">
		</a>
			...
			<c:if test="${order_tableVO.order_status == 0}">
				<tr>
					...
					<td><c:choose>
							<c:when test="${order_tableVO.order_status==0}">
								<a href="#">
									<button type="button" class="button_THREE_TYPE" id="ff">訂單已取消</button>
								</a>
							</c:when>
							<c:otherwise>
	...
	<script type="text/javascript">
		$(document).on("click", ".btn_modal", function() {
			...
			$.ajax({
				url : "/TEA102G120210113/order_table/Order_TableServlet_front",
				type : "GET",
				data : uPair,
				dataType : "JSON",
